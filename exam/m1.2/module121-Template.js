...
class Product {
    ...
    getTotalPrice() {
        ...
        if (this.price < 0 || this.price === 0 && this.quantity < 0 || this.quantity === 0) {
            return undefined
        }
    ...
    sell(quantity) {
        ...
        return { 'name': this.name, 'quantity': this.quantity - quantity }
    }
    restock(quantity) {
        if (quantity <= 0) {
            return undefined
...
console.log(product2.comparePrice(product4));
